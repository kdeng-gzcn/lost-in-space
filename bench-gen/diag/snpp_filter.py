...
def arg_parser():
    parser = argparse.ArgumentParser(
        description="Filter ScanNet++ dataset according to different criteria."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        required=True,
        help="Path to the output directory for the filtered ScanNet++ dataset.",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        required=True,
        help="Path to the data directory for the filtered ScanNet++ dataset.",
    )
    parser.add_argument(
        "--yaml_path",
        type=str,
...
def glob_color_img(hash_dir: Path):
...
def glob_pose(hash_dir: Path):
    ...
    for frame in camera_json["frames"]:
        frame_id_to_pose[Path(frame["file_path"]).stem] = np.array(frame["transform_matrix"])
    for frame in camera_json["test_frames"]:
        frame_id_to_pose[Path(frame["file_path"]).stem] = np.array(frame["transform_matrix"])

    color_imgs = glob_color_img(hash_dir)
    return [frame_id_to_pose[img.stem] for img in color_imgs]
...
def main(cfg):
    ...
    for hash_id, hash_dir in iterate_seq(data_dir):
        logger.info(f"Processing hash: {hash_id}")
        color_imgs = glob_color_img(hash_dir)
        poses = glob_pose(hash_dir)
        ...
        frames = [
            Frame(
                ...
                pose_path=pose,
                hash_id=hash_id,
            ) for color_img, pose in zip(color_imgs, poses)
        ]
        ...
        for frame_pair, sig_dof in frame_pairs:
            ...
            frame_pair.save(output_dir, f"snpp/{sig_dof}", sample_id)
            sample_id += 1

    # Save cfg to actual output dir
    cfg_save_path = output_dir / f"snpp" / "config.yaml"
    try:
